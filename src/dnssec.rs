...
#[cfg(test)]
const ROOT_KSK_2024_KEY_TAG: u16 = 38696;
// Decoded from base64: AwEAAa96jeuknZlaeSrvyAJj6ZHv28hhOKkx3rLGXVaC6rXTsDc449/c...
...
static TRUST_ANCHORS: LazyLock<Vec<DnsRecord>> = LazyLock::new(|| {
    [ROOT_KSK_2017_PUBLIC_KEY, ROOT_KSK_2024_PUBLIC_KEY]
        .into_iter()
        .map(|public_key| DnsRecord::DNSKEY {
            domain: ".".into(),
...
mod tests {
    ...
    #[test]
    fn key_tags_match_published_root_anchors() {
        for (public_key, want_tag) in [
            (ROOT_KSK_2017_PUBLIC_KEY, ROOT_KSK_2017_KEY_TAG),
            (ROOT_KSK_2024_PUBLIC_KEY, ROOT_KSK_2024_KEY_TAG),
        ] {
            let tag = compute_key_tag(ROOT_KSK_FLAGS, 3, ROOT_KSK_ALGORITHM, public_key);
