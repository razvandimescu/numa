...
# as PID 1, builds numa inside, runs four scenarios that CI does not:
#
...
#   D. Without systemd-resolved, the service keeps the original resolv.conf
#      nameserver as its upstream after install rewrites resolv.conf.
#
...
if [ "${NUMA_INSIDE:-}" = "1" ]; then
    ...
    # ---- Scenario D ----
    # Raspberry Pi OS / minimal Debian shape: no systemd-resolved, a plain
    # resolv.conf from DHCP. install rewrites it to 127.0.0.1, so the daemon
    # can only learn the original nameserver from install's backup. The
    # nameserver needn't answer: the assertion is on which upstream is chosen.
    printf "\n=== Scenario D: no systemd-resolved — original nameserver survives install ===\n"
    ...
    mkdir -p /root/.numa
    printf 'nameserver 10.53.0.1\n' > /root/.numa/original-resolv.conf
    printf '# Generated by Numa\nnameserver 127.0.0.1\n' > /etc/resolv.conf
    "$NUMA" uninstall >/dev/null 2>&1 || true
    if grep -q '^nameserver 10.53.0.1' /etc/resolv.conf; then
        pass "uninstall restored resolv.conf from a legacy /root/.numa backup"
    else
        fail "legacy backup not restored: $(tr '\n' ' ' < /etc/resolv.conf)"
    fi
    rm -rf /root/.numa
    reset_state
    printf '%s\n' "$resolv_before" > /etc/resolv.conf
