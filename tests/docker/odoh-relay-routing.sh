...
RELAY_PORT=8443
STACK="$(cd "$(dirname "$0")/odoh-relay" && pwd)"
...
cleanup() {
    ...
    docker compose -f "$STACK/docker-compose.yml" down >/dev/null 2>&1 || true
    rm -f "$STACK/Caddyfile.local"
}
trap cleanup EXIT
...
sed -e 's|^odoh-relay\.example\.com {|:8081 {|' \
    ...
    "$ROOT/packaging/relay/Caddyfile" > "$STACK/Caddyfile.local"
...
CADDY_PORT="$CADDY_PORT" docker compose -f "$STACK/docker-compose.yml" up -d >/dev/null 2>&1
sleep 2
